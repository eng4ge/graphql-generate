...
def parse_graphql_operations(schema_json):
    ...
    def extract_operations(field, parent_path):
        current_path = f"{parent_path} => {field['name']}"
        if parent_path.startswith("Query"):
            operations["queries"].append(current_path)
        elif parent_path.startswith("Mutation"):
            operations["mutations"].append(current_path)
        ...
        if field_type["kind"] in ["OBJECT", "INTERFACE"]:
            ...
            for new_field in new_fields:
                extract_operations(new_field, current_path)
